...
\begin{definition}
  ...
  \[ b_{\text{int}}(i) = 2 + 2i - 3 \sum_{j=0}^i (C_j(8) \pmod 2). \]
\end{definition}
...
\begin{lemma}
  ...
  \begin{proof}
    \leanok
    By induction on $i$. Base case $i=0$: $A_0(8, 2)_2 = 2$. $b_{\text{int}}(0) = 2 + 0 - 3(C_0(8) \pmod 2) = 2 - 0 = 2$.
    Step: $A_{i+1}(8, 2)_2 = A_i(8, 2)_2 + 2 - 3(A_i(8, 2)_1 \pmod 2)$.
    Since $A_j(8, 2)_1 \equiv C_{j+1}(8) \pmod 2$ and $C_0(8) \equiv 0 \pmod 2$, we have:
    \[ \sum_{j=0}^{i} (A_j(8, 2)_1 \pmod 2) = \sum_{j=0}^{i} (C_{j+1}(8) \pmod 2) = \sum_{k=1}^{i+1} (C_k(8) \pmod 2) = \sum_{k=0}^{i+1} (C_k(8) \pmod 2). \]
    Thus the formula holds for $i+1$.
  \end{proof}
\end{lemma}
...
\begin{lemma}
  ...
  \leanok
  We have $b_{\text{int}}(i) = 0$ if and only if $i \equiv 2 \pmod 3$ and $\sum_{j=0}^i (C_j(8) \pmod 2) = \frac{2(i+1)}{3}$.
  \begin{proof}
    \leanok
    $b_{\text{int}}(i) = 0 \iff 2(i+1) = 3 \sum_{j=0}^i (C_j(8) \pmod 2)$.
    Since the RHS is a multiple of 3, we must have $2(i+1) \equiv 0 \pmod 3$, which means $i \equiv 2 \pmod 3$.
...
\begin{theorem}
  ...
  \leanok
  The sequence $A_i(8, 2)$ halts at some index $i$ (meaning $A_i(8, 2)_1$ is odd and $A_i(8, 2)_2 = 0$) if and only if there exists $i$ such that $C_{i+1}(8)$ is odd, $i \equiv 2 \pmod 3$, and $\sum_{j=0}^i (C_j(8) \pmod 2) = \frac{2(i+1)}{3}$.
  \begin{proof}
    \leanok
...
\begin{theorem}
  \label{thm:Aiter_8_2_halt_iff_Diter_7_halt}
  \lean{Aiter_8_2_halt_iff_Diter_7_halt}
  \leanok
  The machine $A$ starting from $(8, 2)$ halts at some index $i$ if and only if there exists $i$ such that $D_{i+1}(7)$ is even, $D_{i+1}(7) \geq 5$, $i \equiv 2 \pmod 3$, and
  \[ \sum_{j=0}^i (D_j(7) \pmod 2) = \frac{i+1}{3}. \]
  \begin{proof}
    \leanok
    By Theorem~\ref{thm:Aiter_8_2_halt_iff_Citer_8_halt}, $A$ halts if and only if $C_{i+1}(8)$ is odd, $i \equiv 2 \pmod 3$, and $\sum_{j=0}^i (C_j(8) \pmod 2) = 2(i+1)/3$.
    By Lemma~\ref{lemma:Citer_succ_eq_Diter_succ}, $C_j(8) = D_j(7) + 1$ for all $j$.
    Thus $C_{i+1}(8)$ is odd if and only if $D_{i+1}(7) + 1$ is odd, which means $D_{i+1}(7)$ is even.
    For the sum, we have:
    \[ \sum_{j=0}^i (C_j(8) \pmod 2) = \sum_{j=0}^i (D_j(7) + 1 \pmod 2). \]
    Since for any $n \in \mathbb{N}$, $(n+1) \pmod 2 = 1 - (n \pmod 2)$, the sum becomes:
    \[ \sum_{j=0}^i (1 - (D_j(7) \pmod 2)) = (i+1) - \sum_{j=0}^i (D_j(7) \pmod 2). \]
    Equating this to $2(i+1)/3$ gives:
    \[ (i+1) - \sum_{j=0}^i (D_j(7) \pmod 2) = \frac{2(i+1)}{3}, \]
    which simplifies to $\sum_{j=0}^i (D_j(7) \pmod 2) = \frac{i+1}{3}$.
  \end{proof}
\end{theorem}
